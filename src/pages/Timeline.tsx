...
import { MapPin, BookOpen, Edit3, X, Users, Globe, Sparkles, Plus, Trash2 } from "lucide-react";
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="flex items-center justify-between pt-3 border-t border-[#D4AF37]/30">
                {onDeleteEvent ? (
                  <button
                    onClick={() => {
                      if (window.confirm(lang === "ar" ? "هل أنت متأكد من حذف هذا الحدث؟" : "Delete this event?")) {
                        onDeleteEvent(selectedEvent.id);
                        setSelectedEvent(null);
                      }
                    }}
                    className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl text-red-600 hover:bg-red-50 dark:hover:bg-red-950/40 text-xs font-semibold cursor-pointer transition-colors"
                  >
                    <Trash2 size={13} />
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-[#D4AF37] hover:bg-[#C5A028] text-[#121110] font-bold shadow text-xs cursor-pointer"
                >
                  <Edit3 size={13} />
