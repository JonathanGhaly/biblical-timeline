...
import { Clock, Calendar, Sparkles, X, BookOpen, Heart } from "lucide-react";
...
export default function TimelinePage({ people, events, lang = "en" }: TimelinePageProps) {
  ...
  // Interactive Hover Pointer State
  ...
  const filteredEvents = useMemo(() => {
    ...
    return validEvents.filter((e) => {
      ...
      const arDescMatch = ((e as any).arabicDescription || "").toLowerCase().includes(term);
      return titleMatch || arMatch || descMatch || arDescMatch;
    });
  }, [validEvents, term]);
  ...
  // Helper getters for localized fields
  const getPersonDescription = (person: Person): string => {
    const pAny = person as any;
    if (isRTL) return pAny.arabicNotes || person.notes || pAny.description || "";
    return person.notes || pAny.description || pAny.arabicNotes || "";
  };

  const getMarriageTitle = (marriage: MarriageItem): string => {
    return isRTL ? marriage.arabicTitle : marriage.title;
  };

  const getMarriageDescription = (marriage: MarriageItem): string => {
    return isRTL
  ...
  return (
    <div className="space-y-6 animate-fadeIn" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-2xl border-2 border-[#D4AF37] bg-gradient-to-r from-[#800020]/15 via-[#FBF8EF] to-[#1A365D]/15 dark:from-[#1C1A17] dark:via-[#161412] dark:to-[#1A365D]/25 shadow-md">
        <div>
          ...
          <p className="text-xs sm:text-sm text-[#6B5E4E] dark:text-[#A99F8D] mt-1">
            {isRTL
              ? "استكشف تداخل أعمار الآباء والأحداث التاريخية مع تفاصيل الوصف المترجمة عبر الخط الزمني الأفقي."
              : "Horizontal chart visualizing overlapping patriarch lifespans, localized events, and biblical descriptions."}
          </p>
        </div>
            ...
            >
              ...
              {hoverState && (
                <>
                  ...
                  />
                  <div
                    className="absolute pointer-events-none z-40 px-2.5 py-1 rounded-md bg-[#800020] text-[#F3E5AB] text-[11px] font-bold font-mono shadow-lg border border-[#D4AF37] whitespace-nowrap transition-transform duration-75 flex items-center gap-1.5"
                    style={{
                  ...
                  >
                    ...
                    <span>{formatYearDisplay(hoverState.year, lang)}</span>
                  </div>
                </>
              )}
              ...
              {/* SECTION 1: Biblical Patriarchs Lifespan Bars */}
              <div className="space-y-2 relative z-10 mb-8 pt-2">
                ...
                {filteredPeople.map(({ person, birthYear, deathYear, duration }) => {
                  ...
                  const desc = getPersonDescription(person);
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="relative flex-1 h-full">
                        <div
                          className="absolute top-1 bottom-1 rounded-md bg-gradient-to-r from-[#D4AF37] to-[#C5A028] text-[#121110] px-2 flex items-center justify-between text-[10px] font-bold shadow-sm transition-transform group-hover:scale-y-110 border border-[#8C6F12]"
                          style={{
                            [isRTL ? "right" : "left"]: `${pos}px`,
                            width: `${widthPx}px`,
                          }}
                          title={desc ? `${displayName}: ${desc}` : displayName}
                        >
              ...
              {derivedMarriages.length > 0 && (
                <div className="space-y-2 relative z-10 my-8 pt-4 border-t border-[#D4AF37]/30">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-[#800020] dark:text-[#F3E5AB] mb-2 px-2 flex items-center gap-1.5">
                    <Heart size={14} className="text-[#800020] dark:text-[#D4AF37]" />
                    <span>{isRTL ? "الأزواج والزيجات" : "Biblical Marriages"} ({derivedMarriages.length})</span>
                  </h4>

                  {derivedMarriages.map((marriage) => {
                    const title = getMarriageTitle(marriage);
                    const desc = getMarriageDescription(marriage);
                    const pos = getPosPx(marriage.year);

                    return (
                      <div
                        key={marriage.id}
                        className="flex items-center h-7 hover:bg-[#800020]/10 rounded-lg transition-colors cursor-pointer"
                        onClick={() => setSelectedItem({ type: "marriage", data: marriage })}
                      >
                        <div
                          className={`sticky ${
                            isRTL ? "right-0 text-right" : "left-0 text-left"
                          } z-20 w-44 shrink-0 px-3 py-1 text-xs font-semibold truncate text-[#800020] dark:text-[#F3E5AB] bg-white/90 dark:bg-[#1C1A17]/90 backdrop-blur-sm border-r border-l border-[#D4AF37]/30 shadow-sm`}
                          style={{ marginInlineStart: "-176px" }}
                          title={title}
                        >
                          {title}
                        ...
                        <div className="relative flex-1 h-full">
                          <div
                            className="absolute top-0.5 bottom-0.5 px-2.5 rounded-full bg-gradient-to-r from-[#D4AF37] to-[#B8860B] text-[#121110] text-[10px] font-bold flex items-center gap-1 shadow-sm border border-[#8C6F12]"
                            style={{ [isRTL ? "right" : "left"]: `${pos}px` }}
                            title={`${title}: ${desc}`}
                          >
                            <Heart size={10} className="fill-current text-[#800020]" />
              ...
              <div className="space-y-2 relative z-10 pt-4 border-t border-[#D4AF37]/30">
                ...
                {filteredEvents.map((event) => {
                  ...
                  return (
                    ...
                    >
                      {/* Sticky Pinned Event Label Column */}
                      <div
                        className={`sticky ${
                          isRTL ? "right-0 text-right" : "left-0 text-left"
                        ...
                        style={{ marginInlineStart: "-176px" }}
                        title={`${displayTitle}${displayDesc ? ` - ${displayDesc}` : ""}`}
                      >
                        {displayTitle}
                      </div>
                      ...
                      <div className="relative flex-1 h-full">
                        <div
                          ...
                          style={{ [isRTL ? "right" : "left"]: `${pos}px` }}
                          title={`${displayTitle}: ${displayDesc}`}
                        >
      ...
      {/* Fully Localized Item Detail Modal */}
      {selectedItem && (
        <div
          ...
          >
              ...
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xl font-bold font-cinzel text-[#800020] dark:text-[#F3E5AB]">
                  {selectedItem.type === "person"
                    ...
                    : getMarriageTitle(selectedItem.data)}
                </h3>
              </div>
              <button
                onClick={() => setSelectedItem(null)}
            ...
            {selectedItem.type === "person" && (
              <div className="space-y-3 text-xs">
                ...
                {getPersonDescription(selectedItem.data) && (
                  <div className="bg-[#D4AF37]/10 p-3 rounded-xl border border-[#D4AF37]/20 text-xs leading-relaxed">
                    <p className="font-semibold text-[#800020] dark:text-[#D4AF37] mb-1">
                      {isRTL ? "الوصف والملاحظات:" : "Description & Notes:"}
                    </p>
                    <p className="italic">{getPersonDescription(selectedItem.data)}</p>
                  </div>
                )}
            ...
            {selectedItem.type === "event" && (
              <div className="space-y-3 text-xs">
                ...
                {getEventDisplayDescription(selectedItem.data, lang) && (
                  <div className="bg-[#D4AF37]/10 p-3 rounded-xl border border-[#D4AF37]/20 text-xs leading-relaxed">
                    <p className="font-semibold text-[#800020] dark:text-[#D4AF37] mb-1">
                      {isRTL ? "تفاصيل الحدث:" : "Event Description:"}
                    </p>
                    <p className="italic">{getEventDisplayDescription(selectedItem.data, lang)}</p>
                  </div>
                )}
            ...
            {/* Marriage Modal Content */}
            {selectedItem.type === "marriage" && (
              <div className="space-y-3 text-xs">
                <p>
                  <strong>{t.year}:</strong> {formatYearDisplay(selectedItem.data.year, lang)}
                </p>
                {selectedItem.data.husbandAge !== undefined && (
                  <p>
                    <strong>{isRTL ? "عمر الزوج عند الزواج:" : "Husband Age at Marriage"}:</strong> {selectedItem.data.husbandAge} {t.years}
                  </p>
                )}
                {selectedItem.data.wifeAge !== undefined && (
                  <p>
                    <strong>{isRTL ? "عمر الزوجة عند الزواج:" : "Wife Age at Marriage"}:</strong> {selectedItem.data.wifeAge} {t.years}
                  </p>
                )}

                {getMarriageDescription(selectedItem.data) && (
                  <div className="bg-[#D4AF37]/10 p-3 rounded-xl border border-[#D4AF37]/20 text-xs leading-relaxed">
                    <p className="font-semibold text-[#800020] dark:text-[#D4AF37] mb-1">
                      {isRTL ? "تفاصيل الزواج:" : "Marriage Description:"}
                    </p>
                    <p className="italic">{getMarriageDescription(selectedItem.data)}</p>
                  </div>
                )}

                {selectedItem.data.references && selectedItem.data.references.length > 0 && (
                  <p className="flex items-center gap-1 font-semibold text-[#800020] dark:text-[#D4AF37]">
                    <BookOpen size={13} />
                    <span>{selectedItem.data.references.join(", ")}</span>
                  </p>
                )}
              </div>
            )}
